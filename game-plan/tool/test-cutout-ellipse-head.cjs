...
assert.equal(C.fadeAlpha(part,100,100),1,'Ellipse endpoint stays opaque');
assert.equal(C.fadeAlpha(part,100,60),0,'Curved ellipse edge is fully transparent');
assert.equal(C.fadeAlpha(part,100,80),C.fadeAlpha(part,105,100),'Equal normalized ellipse radii have equal alpha');
assert.ok(Math.abs(C.fadeAlpha(part,100,77)-.5)<1e-10,'Half of the distance remaining after Náběh has exactly half alpha');
assert.ok(Math.abs(C.fadeAlpha(part,105.75,100)-.5)<1e-10,'R2 uses the same proportional ellipse distance as R1');
...
const rectangle={...part,joint_fade:{start:{...part.joint_fade.start,shape:'rectangle'}}};
assert.equal(C.fadeAlpha(rectangle,100,100),1,'Rectangle endpoint stays opaque');
assert.equal(C.fadeAlpha(rectangle,100,60),0,'Rectangle far edge is fully transparent');
assert.equal(C.fadeAlpha(rectangle,109,80),C.fadeAlpha(rectangle,100,80),'Rectangle fades in straight bands across its width');
assert.equal(C.fadeAlpha(rectangle,111,80),1,'Rectangle does not affect pixels beyond R2');
assert.notEqual(C.fadeAlpha(rectangle,109,80),C.fadeAlpha(part,109,80),'Rectangle corners differ from ellipse');
const skin={parts:{head:part},layers:['head']};
let changed=E.fadeChange(clip,skin,0,'head','start',{radius2:25,onset:.35},'frame');
...
assert.equal(C.fadeFor(C.partFor(changed.skin,'head',C.sample(changed.clip,0))).onset,.35);
assert.equal(C.fadeFor(C.partFor(changed.skin,'head',C.sample(changed.clip,1))).onset,.15);
assert.equal(C.fadeFor(C.partFor(changed.skin,'head',C.sample(changed.clip,.5))).onset,.25);
...
assert.equal(C.fadeFor(legacy).shape,'ellipse','Old masks default to ellipse');
assert.equal(C.fadeFor(legacy).onset,.15,'Old masks keep the current 15% default onset');
...
assert.ok(Math.abs(view.stageSize(null,900,1400).height-900*view.DEFAULT_VIEW/512)<1e-6);
...
console.log('PASS: bounded ellipse/rectangle fades, legacy circles, scoped axes, independent head/neck and torso manipulation.');
